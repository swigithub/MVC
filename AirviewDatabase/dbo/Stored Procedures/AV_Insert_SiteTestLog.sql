...
CREATE PROCEDURE [dbo].[AV_Insert_SiteTestLog] 
...
BEGIN
	...
	SET @lSectorId=(SELECT TOP 1 x.SectorRefId FROM @List x)
	
		DECLARE @logNetMode AS NVARCHAR(15)=''
		DECLARE @xNetMode AS NVARCHAR(50)=''
		
		
			
			IF (SELECT COUNT(DISTINCT x.NetworkMode) FROM @List x)=1
			BEGIN
				SELECT DISTINCT @xNetMode=x.NetworkMode FROM @List x
				...
				BEGIN
					...
					SET @NetworkMode='NR'
				END
				ELSE IF @xNetMode='LTE'
				BEGIN
					SET @logNetMode='LTE'
					SET @NetworkMode='LTE'
				END
				ELSE IF @xNetMode='WCDMA'
				BEGIN
					SET @logNetMode='WCDMA'
					SET @NetworkMode='WCDMA'
				END
				ELSE IF @xNetMode='GSM'
				BEGIN
					SET @logNetMode='GSM'
					SET @NetworkMode='GSM'
				END
	...
	BEGIN
		...
		BEGIN
			...
			DECLARE @maxSrId AS INT=0
			
			SELECT TOP 1 @maxSrId=SrId FROM AV_SiteOoklaLogs
			where SiteId=@SiteId and NetworkModeId=@lNetModeId and BandId=@lBandId and CarrierId = @lCarrierId and SectorId=@lSectorId
			ORDER BY SrId DESC
			
			UPDATE AV_SiteOoklaLogs
			SET Latitude = (SELECT TOP 1 x.Latitude FROM @List x ORDER BY x.TimeStamp DESC), 
			Longitude = (SELECT TOP 1 x.Longitude FROM @List x ORDER BY x.TimeStamp DESC),
			NetworkMode = @logNetMode,
			RSRP = (CASE WHEN @logNetMode='NR' THEN ISNULL((SELECT AVG(x.NRRsrp) FROM @List x WHERE x.NRRsrp<>0),0) ELSE ISNULL((SELECT AVG(x.LteRsrp) FROM @List x WHERE x.LteRsrp<>0),0) END),
			RSRQ = (CASE WHEN @logNetMode='NR' THEN ISNULL((SELECT AVG(x.NRRsrq) FROM @List x WHERE x.NRRsrq<>0),0) ELSE ISNULL((SELECT AVG(x.LteRsrq) FROM @List x WHERE x.LteRsrq<>0),0) END),
			RSNR = (CASE WHEN @logNetMode='NR' THEN ISNULL((SELECT AVG(x.NRRsnr) FROM @List x WHERE x.NRRsnr<>0),0) ELSE ISNULL((SELECT AVG(x.LteRsnr) FROM @List x WHERE x.LteRsnr<>0),0) END)
			WHERE SiteId=@SiteId and NetworkModeId=@lNetModeId and BandId=@lBandId and CarrierId = @lCarrierId and SectorId=@lSectorId
			AND Latitude IS NULL AND Longitude IS NULL AND NetworkMode IS NULL;
			...
			--SELECT @woID,NULL,@lNetModeId,@lBandId,@lCarrierId,@lSectorId,
			--(SELECT TOP 1 x.Latitude FROM @List x ORDER BY x.TimeStamp DESC),
			--(SELECT TOP 1 x.Longitude FROM @List x ORDER BY x.TimeStamp DESC),NULL,NULL,NULL,NULL,@logNetMode,
			--ISNULL((SELECT AVG(x.LteRsrp) FROM @List x WHERE x.LteRsrp<>0),0),
			--ISNULL((SELECT AVG(x.LteRsrq) FROM @List x WHERE x.LteRsrq<>0),0),
			--ISNULL((SELECT AVG(x.LteRsnr) FROM @List x WHERE x.LteRsnr<>0),0);
		END
	...
	IF (@TestType='CW' OR @TestType='CCW')
		...
		DECLARE @PCIList TABLE(NetworkMode nvarchar(15),PCIID int,PCIColor NVARCHAR(50))
		...
		IF EXISTS(SELECT TOP 1 y.SiteId FROM AV_SiteTestLog AS y WHERE y.SiteId=@SiteId AND y.NetworkModeId=@cwNetModeID AND y.BandId=@cwBandID  AND y.TestType IN('CW','CCW'))
		BEGIN
			--INSERT INTO @PCIList(PCIID,PCIColor)
			--SELECT *
			--FROM
			--(
			--	SELECT DISTINCT y.PciId,y.pciColor 'PCIColor'
			--	FROM AV_SiteTestLog y
			--	WHERE y.SiteId=@SiteId AND y.NetworkModeId=@cwNetModeID AND y.BandId=@cwBandID AND y.CarrierId=@cwCarrierID
			--	AND y.TestType IN('CW','CCW')
			--) x
			
			INSERT INTO @PCIList(NetworkMode,PCIID,PCIColor)
			SELECT *
			FROM
			(
				SELECT DISTINCT x.NetworkMode,(CASE WHEN x.NetworkMode='NR' then x.NRPCI else x.PciId END) 'PCIID',y.pciColor 'PCIColor'
				FROM @List x
				LEFT OUTER JOIN AV_SiteTestLog y ON y.SiteId=x.SiteRefId
				AND (CASE WHEN x.NetworkMode='NR' then x.NRPCI else x.PciId END)=(CASE WHEN y.NetworkMode='NR' then y.NRPCI else y.PciId END)
				WHERE y.SiteId=@SiteId AND y.NetworkModeId=@cwNetModeID AND y.BandId=@cwBandID AND y.CarrierId=@cwCarrierID
				AND y.TestType IN('CW','CCW')
			) x
		END
		...
		---------------------PCI Plot Start------------------------
		DECLARE @tPciID AS NVARCHAR(15)=''
		DECLARE @tPciColor AS NVARCHAR(50)=''
		DECLARE @tNetworkMode AS NVARCHAR(15)=''	
		DECLARE db_cluster3 CURSOR FOR  
		SELECT NetworkMode,PCIID,PCIColor FROM @PCIList AS cl WHERE cl.PCIColor IS NULL
		OPEN db_cluster3 
		FETCH NEXT FROM db_cluster3 INTO @tNetworkMode,@tPciID,@tPciColor
		WHILE @@FETCH_STATUS = 0   
		BEGIN   
			DECLARE @pciColorCode as nvarchar(50)=''
			DECLARE @pciRandColorCode AS NVARCHAR(50)=''			
			
			IF (@tPciColor='' OR @tPciColor IS NULL)
			BEGIN
				IF EXISTS(SELECT y.SectorId FROM AV_Sectors y WHERE y.SiteId=@SiteId AND y.NetworkModeId=@cwNetModeID AND y.BandId=@cwBandID AND y.CarrierId=@cwCarrierID AND y.PCI=@tPciID)
				BEGIN
					SELECT @pciRandColorCode=y.sectorColor
					FROM AV_Sectors y
					WHERE y.SiteId=@SiteId AND y.NetworkModeId=@cwNetModeID AND y.BandId=@cwBandID
					AND y.CarrierId=@cwCarrierID AND y.PCI=@tPciID
				END
				ELSE
				BEGIN
					IF @tNetworkMode='NR'
					BEGIN									
						SELECT TOP 1 @pciRandColorCode=x.ColorCode
						FROM AD_Definations x
						WHERE x.DefinationTypeId=@ColorDefTypeId				
						AND x.ColorCode NOT IN(SELECT DISTINCT x.PCIColor FROM @PCIList x WHERE x.PCIColor IS NOT NULL)
						AND x.IsActive=1			
						ORDER BY NEWID()
					END
					ELSE
					BEGIN
						SET @pciRandColorCode='#87898A';
					END
				END
				
				UPDATE @PCIList
				SET PCIColor= @pciRandColorCode
				WHERE PCIID=@tPciID
			END	
		FETCH NEXT FROM db_cluster3 INTO @tNetworkMode,@tPciID,@tPciColor
		END   
		...
		(
			...
			FROM
			(
				SELECT x.Site,x.Sector,x.NetworkMode,x.Band,x.Carrier,x.CellId,x.LacId,
				(CASE WHEN x.NetworkMode='NR' then x.NRPCI else x.PciId END) 'PciId',
				x.MccId,x.MncId,
				x.Latitude,x.Longitude,x.TimeStamp,
				x.GsmRssi,x.GsmRxQual,x.WcdmaRscp,x.WcdmaEcio,x.LteRsrp,x.LteRsrq,x.LteRsnr,x.LteCqi,x.DistanceFromSite,x.AngleToSite,x.FtpStatus,
				x.StackTrace,x.TestResult,x.SubNetworkMode,x.ActualBand,x.ActualCarrier, --x.IsHandover
				ISNULL((SELECT TOP 1 (CASE WHEN y.NetworkMode='NR' then y.NRPCI else y.PciId END) FROM #tmpMobility y WHERE y.Site=x.Site AND y.rowId=x.rowId-1 ORDER BY y.rowId desc),-1) 'prvPCI',x.FloorId,x.RRCState,
		...
		(
			...
			ISNULL((SELECT TOP 1 (CASE WHEN y.NetworkMode='NR' then y.NRPCI else y.PciId END) FROM #tmpMobility y WHERE y.Site=x.Site AND y.rowId=x.rowId-1 ORDER BY y.rowId desc),-1) 'prvPCI',
			(CASE WHEN x.NetworkMode='NR' then x.NRPCI else x.PciId END) 'nxtPCI',@TestType 'DriveType'
			FROM #tmpMobility x			
		) x
		...
		--CASE WHEN x.isIntraSitePCI=1 THEN 
		--(SELECT TOP 1 cl.PCIColor FROM @PCIList AS cl WHERE cl.PCIID=(CASE WHEN x.NetworkMode='NR' then x.NRPCI else x.PciId END)) 'pciColor',
		--WHEN x.isIntraSitePCI=0 THEN ISNULL((SELECT DISTINCT TOP 1 z.SectorColor FROM AV_MarketSites z WHERE z.ClientId=@ClientId AND z.CityId=@CityId AND z.NetworkModeId=@cwNetModeID AND z.BandId=@cwBandID AND z.CarrierId=@cwCarrierID AND z.PCI=x.PciId),'#87898A') END 'pciColor',
		...
		(SELECT TOP 1 cl.CarrierColor FROM @CarrierList AS cl WHERE cl.Carrier=(CASE WHEN x.NetworkMode='NR' then x.NRCarrier else x.Carrier END)) 'chColor', CAST(@acceptLog AS BIT) 'isActive',x.FloorId,x.RRCState,
		x.NRBand,x.NRCarrier,x.NRRsrp,x.NRRsrq,x.NRRsnr,x.NRCqi,x.NRPci,x.FromPCI,x.ToPCI

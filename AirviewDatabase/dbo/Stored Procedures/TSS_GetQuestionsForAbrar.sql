...
-- =============================================
CREATE PROCEDURE [dbo].[TSS_GetQuestionsForAbrar]
@Filter NVARCHAR(50)
...
-- [dbo].[TSS_GetQuestions]	'GET_BY_SECTIONID',10047
	...
	BEGIN
	SELECT tq.*,tsq.SiteQuestionId,case when tr.ResponseId > 0 then 0 else 1 end [IsQuestion],tr.*,def.DefinationName 'QuestionType', tsq.IsInclude, tsq.SiteSectionId
		FROM TSS_Questions AS tq
		INNER JOIN AD_Definations AS def ON def.DefinationId=tq.QuestionTypeId
		INNER JOIN TSS_SiteQuestions AS tsq ON tsq.QuestionId=tq.QuestionId and tsq.SiteSectionId = @Value and tsq.SiteSurveyId = @surveyId
		--INNER JOIN TSS_SiteSections AS tss ON TSS.SiteSectionId = TSQ.SiteSectionId
	...
	SELECT Distinct tq.Question,tq.QuestionTypeId,tq.IsRepeatable,tq.IsRequired,tq.IsImageRequired
		  ...
		  tq.DynamicRows,tq.DynamicRowsCount,
		   ...
		   tsq.SiteQuestionId,case when tr.ResponseId > 0 then 0 else 1 end [IsQuestion],tr.UserValues,tr.MaxValue,tr.MinValue,tr.IsReadOnly,tr.QuestionId,tr.IsGps,def.DefinationName 'QuestionType',
		...
		else (case when isnull(tsr.ResponseId,0)=0 and tq.QuestionTypeId = 243513  then 1 else 0 end)
	    end )) as IsChecked,
		(
		case when tq.QuestionTypeId = 243513 and tsr.ResponseId is not null  

...
function addtoSurveyList() {
    debugger;
    var Svalues = $("#hfSId").val();
    var SSvalues = $("#hfSurveyId").val();
    if (Svalues != undefined) {
        var sId = Svalues.split(",");
        var ssId = SSvalues.split(",");
        $.each(sId, function (index, value) {
            $.each(Surveys, function (i, v) {
                if (v.SurveyId == parseInt(value)) {
                    $("#listOfSurveys p").append('<span class="SurveyList">' + v.SurveyTitle + ' &nbsp; <i class="fa fa-remove delSurvey" data-SId=' + parseInt(ssId[index]) + '></i>  &nbsp;</span><br/>')
                }
            });
        });
    }
...
function NewRow(e) {
    var RowsCount = $('.SectorNewDeviceRow');
    var str = NewDeviceRow.replace("Band-r0", "Band-r" + count + "");

    var RowsFields = (str.match(new RegExp("r0", "g")) || []).length;

    for (var i = 0; i < RowsFields; i++) {
        str = str.replace("r0", "r" + RowsCount.length + "");
    }
                ...
                $('#tbl-wo tbody tr').each(function () {
                    ...
                    if (networkmodetext == "14") {
                        if (pcivalue < 0) {
                            ...
                            alert("Please Enter PCI Value > 0");

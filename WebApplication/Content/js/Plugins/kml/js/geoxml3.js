...
geoXML3.parser = function (options) {
    ...
    var createPolyline = function (placemark, doc) {
        ...
        if (!parserOptions.suppressInfoWindows) {
            var Pname = "'" + placemark.name + "'";
            var abc = "";
            var infoWindowOptions = geoXML3.combineOptions(parserOptions.infoWindowOptions, {
                content: '<div class="geoxml3_infowindow">' + abc +
                         '<div>' + placemark.description + '</div><div onclick="DeleteRoute(' + Pname + ')" class="btn btn-success">Delete</div>&nbsp;<div onclick="RouteDirection(' + Pname + ')" class="btn btn-primary">Direction</div></div>',
                pixelOffset: new google.maps.Size(0, 2)
            });
            if (parserOptions.infoWindow) {
